  ...
  if(m_outdatedAURPackageList->count() > 0)
  {
    ...
    else
    {
      ...
      m_toolButtonAUR->setToolTip(
          StrConstants::getNewUpdates(m_outdatedAURPackagesNameVersion->count()).arg(m_outdatedAURPackagesNameVersion->count()));
    }
...
void MainWindow::refreshStatusBar()
{
  ...
  if((m_numberOfOutdatedPackages > 0) && (!isAURGroupSelected()))
  {
    ...
    else
    {
      ...
      m_toolButtonPacman->setToolTip(StrConstants::getNewUpdates(m_numberOfOutdatedPackages).arg(m_numberOfOutdatedPackages));
    }
